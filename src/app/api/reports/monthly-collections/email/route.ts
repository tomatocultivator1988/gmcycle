...
export async function POST() {
  try {
    ...
    const html = `
      <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:640px;margin:0 auto;background:#fff;padding:24px;">
        <div style="text-align:center;border-bottom:2px solid #991b1b;padding-bottom:16px;margin-bottom:20px;">
          ...
          <p style="font-size:16px;font-weight:600;color:#991b1b;margin:8px 0 0;">Monthly Collection Report</p>
        </div>

        <div style="margin-bottom:20px;">
          <h3 style="font-size:14px;color:#475569;margin:0 0 8px;">Summary</h3>
          <table style="width:100%;border-collapse:collapse;font-size:13px;">
            <tr><td style="padding:4px 0;color:#64748b;">Month</td><td style="padding:4px 0;text-align:right;font-weight:700;">${monthLabel}</td></tr>
        ...
        </table>` : "<p style='color:#94a3b8;font-size:13px;'>No collections this month.</p>"}
    ...
    await sendEmail({
      to: toEmail,
      subject: `Monthly Collection Report — ${monthLabel}`,
      html,
    });

    return NextResponse.json({ message: "Monthly collection report emailed", total: total.toFixed(2), count: payments.length, month: monthLabel });
  } catch (error) {
