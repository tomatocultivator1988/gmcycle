  ...
  try {
    ...
    const rowsHtml = payments.map((p) => `
      <tr>
        ...
        <td style="padding:6px 8px;border-bottom:1px solid #e2e8f0;text-align:right;">${dateToManilaDateOnly(p.paymentDate)}</td>
    ...
    const html = `
      <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:640px;margin:0 auto;background:#fff;padding:24px;">
        <div style="text-align:center;border-bottom:2px solid #991b1b;padding-bottom:16px;margin-bottom:20px;">
          ...
          <p style="font-size:16px;font-weight:600;color:#991b1b;margin:8px 0 0;">Collection Report</p>
        </div>

        <div style="margin-bottom:20px;">
          <h3 style="font-size:14px;color:#475569;margin:0 0 8px;">Summary</h3>
          <table style="width:100%;border-collapse:collapse;font-size:13px;">
            <tr><td style="padding:4px 0;color:#64748b;">Total Collections</td><td style="padding:4px 0;text-align:right;font-weight:700;color:#059669;">${formatPeso(total.toFixed(2))}</td></tr>
            <tr><td style="padding:4px 0;color:#64748b;">Total Payments</td><td style="padding:4px 0;text-align:right;font-weight:700;">${payments.length}</td></tr>
          </table>
        </div>
    ...
    await sendEmail({
      to: toEmail,
      subject: "Collection Report — MyFaveGadgets",
      html,
    });

    return NextResponse.json({ message: "Collection report emailed", total: total.toFixed(2), count: payments.length });
  } catch (error) {
